-- 浮窗持久缓存
_G.afterglow_color_buf = nil
_G.afterglow_color_win = nil
_G.afterglow_last_hl_group = _G.afterglow_last_hl_group or nil

-- 显示颜色贴纸浮窗
function _G.ShowAfterglowColor()
	...
	if fg == "" then
		...
		character = color_to_character[fg] or "Afterglow"
	...
	local afterglow_colors = {
		["#EE0022"] = { name = "Ran", hl = "AfterglowRan", hex = "#EE0022" },
		["#00CCAA"] = { name = "Moca", hl = "AfterglowMoca", hex = "#00CCAA" },
		["#FF9999"] = { name = "Himari", hl = "AfterglowHimari", hex = "#FF9999" },
		["#FFEEBB"] = { name = "Tsugumi", hl = "AfterglowTsugumi", hex = "#FFEEBB" },
		["#BB0033"] = { name = "Tomoe", hl = "AfterglowTomoe", hex = "#BB0033" },
		["#E53344"] = { name = "Afterglow", hl = "Afterglow", hex = "#E53344" },
	}

	for _, info in pairs(afterglow_colors) do
		vim.cmd(string.format("highlight! %s guifg=%s gui=bold", info.hl, info.hex))
	end

	local info = afterglow_colors[fg]
	local hl_group = info and info.hl

	if hl_group == _G.afterglow_last_hl_group then
		return
	end
	_G.afterglow_last_hl_group = hl_group

	if _G.afterglow_color_buf and vim.api.nvim_buf_is_valid(_G.afterglow_color_buf)
		and _G.afterglow_color_win and vim.api.nvim_win_is_valid(_G.afterglow_color_win) then
		vim.api.nvim_buf_set_lines(_G.afterglow_color_buf, 0, -1, false, msg)

		if hl_group then
			vim.defer_fn(function()
				vim.api.nvim_buf_add_highlight(_G.afterglow_color_buf, -1, hl_group, 0, 6, -1)
				vim.api.nvim_buf_add_highlight(_G.afterglow_color_buf, -1, hl_group, 1, 6, -1)
			end, 20)
		end
	else
		...
		_G.afterglow_color_buf = buf
		_G.afterglow_color_win = win
	end

	if not _G.afterglow_sticker_enabled then
		return
	end

	local base_path = os.getenv("HOME") .. "/.config/nvim/themes/BanGDream_vim_theme/Afterglow/Afterglow_sticker"
...
do
	...
	if #init_lines > 0 and init_lines[1] == "true" then
		_G.afterglow_sticker_enabled = true
	else
		_G.afterglow_sticker_enabled = false
	end
end
...
vim.keymap.set("n", "<leader>n", function()
	...
	desc = "切换 Afterglow 贴纸开关 + 刷新 wezterm",
	silent = true,
})
